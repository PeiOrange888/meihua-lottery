...
双色球和大乐透分别建立独立的号码池：双色球为红球 `1–33`、蓝球 `1–16`，大乐透为前区 `1–35`、后区 `1–12`。应用将年支、农历日期、时辰、动爻、本卦、变卦、互卦和体用关系组成卦象种子，再为四个号码池分别使用独立域进行确定性 Fisher–Yates 洗牌，最后取前 6、1、5、2 个号码并排序展示。

当前算法版本为 `deterministic-shuffle-v1`。这种方式仍然保证同一卦象可复现，但不再把 64 卦序直接对 33 或 35 取模，也不依赖重复号码的偏移来“补位”。每个合法号码都来自完整号码池；它仍然只是娱乐映射，不代表真实随机开奖的概率优势。
...
算法升级后，浏览器会校验时辰缓存中的算法版本并自动丢弃旧版本缓存；Firebase 中已经保存的历史预测记录不会被重写。
