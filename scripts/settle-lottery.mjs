...
async function settleType(db, type) {
  const branch = normalizeBranch(db[type]);
  ...
  if (!DRY_RUN) {
    await fetchJson(FIREBASE_URL.replace('.json', `/${type}.json`), {
      ...
      body: JSON.stringify(serializeBranch(branch))
    });
  }
...
async function main() {
  const db = await fetchJson(FIREBASE_URL);
  const results = [];
  for (const type of TYPES) {
    results.push(await settleType(db || {}, type));
  }
